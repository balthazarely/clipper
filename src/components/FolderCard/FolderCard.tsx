...
export function FolderCard({
  ...
  const contextMenuItems: ContextMenuItem[] = [
    ...
    ...(onDelete ? [{ label: "Delete", onClick: () => setShowDeleteConfirm(true), isDanger: true }] : []),
  ];
                ...
                <div className="w-12 h-12 rounded-2xl flex items-center justify-center" style={{ background: editColor + "20" }}>
                  <svg
                    ...
                    dangerouslySetInnerHTML={{
                      __html: GROUP_ICONS.find((i) => i.id === editIcon)?.svg ?? GROUP_ICONS.find((i) => i.id === "briefcase")!.svg,
                    }}
                  />
                </div>
              </div>
                ...
                {GROUP_ICONS.map((ic) => (
                  ...
                  >
                    <svg
                      ...
                      viewBox="0 0 24 24"
                      fill="none"
                      stroke={editIcon === ic.id ? editColor : "#9ca3af"}
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
        ...
        {showDeleteConfirm && (
          <>
            ...
            <motion.div
              ...
              className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-xl z-50 w-full max-w-xs mx-4 p-5"
            >
              <h3 className="text-sm font-semibold text-gray-900 mb-2">Delete folder?</h3>
